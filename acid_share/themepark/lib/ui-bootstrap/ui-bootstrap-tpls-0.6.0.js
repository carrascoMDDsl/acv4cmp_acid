      ...
      return {
        ...
        link: function link ( scope, element, attrs ) {
          ...
          scope.$on('$destroy', function onDestroyTooltip() {
            ...
            /* ACV OJO OJO ADDED 20140108 to catch class change in button signaling a disable to close the tooptip
            BEGIN
             */

            if( attrs[ "ngDisabled"] || attrs[ "ngIf"]  || attrs[ "ngShow"]) {
                if( ( element.context.nodeType == 1) &&  (
                    ( ( element.context.nodeName.toUpperCase() == "INPUT") && !( element.context.getAttribute("type") && ( element.context.type.toUpperCase() == "CHECKBOX"))) ||
                    ( element.context.nodeName.toUpperCase() == "SELECT") ||
                    ( element.context.nodeName.toUpperCase() == "TEXTAREA") ||
                    ( element.context.nodeName.toUpperCase() == "BUTTON"))) {

                    var pAttributeChangeHandler = function( mutation) {
                        if ( scope.tt_isOpen ) {
                            hide();
                        }
                    }
                    ...
                    if( WebKitMutationObserver) {
                        var observer = new WebKitMutationObserver(
                            function ( theMutations) {
                                theMutations.forEach( pAttributeChangeHandler);
                            }
                        );
                        observer.observe( element.context, { attributes: true, subtree: false });
                    }
                    else {
                        element.context.addEventListener("DOMAttrModified", pAttributeChangeHandler);
                    }
                }
            }
